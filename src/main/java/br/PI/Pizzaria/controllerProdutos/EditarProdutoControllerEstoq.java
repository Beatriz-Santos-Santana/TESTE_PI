...
@Controller
public class EditarProdutoControllerEstoq {

    @Autowired
    private ProdutoRepository produtoRepository;

    // Método para exibir a página de edição do produto
    @GetMapping("/editar-produto-estoq/{id}")
    public String editarProduto(@PathVariable Long id, Model model) {
        Produto produto = produtoRepository.findById(id).orElseThrow(() -> new RuntimeException("Produto não encontrado"));
        model.addAttribute("produto", produto);
        return "editarProdutoEstoq"; // Nome da página de edição
    }

    // Método para salvar as alterações no produto
    @PostMapping("/editar-produto-estoq/{id}")
    public String salvarAlteracoes(@PathVariable Long id, @ModelAttribute Produto produto) {
        ...
        return "redirect:/listar-produtos-estoq"; // Redireciona para a lista de produtos após salvar
    }
